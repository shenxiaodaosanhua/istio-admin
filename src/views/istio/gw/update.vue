...
import { getGwDetailByNsAndName, updateGw } from '@/api/gw'
...
export default {
  ...
  created() {
    ...
    getGwDetailByNsAndName(this.$route.params.ns, this.$route.params.name).then(res => {
      this.form = res.data
      const selector = this.form.spec.selector
  ...
  methods: {
    submitForm() {
      updateGw(this.form).then(res => {
        if (res.data === 'ok') {
          this.$message.success('保存成功')
          this.$router.replace('/istio/gw/update')
        }
      })
    },
    parseSelector() {
