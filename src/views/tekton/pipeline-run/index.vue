...
export default {
  ...
  created() {
    ...
    getPipelineRunByNs(this.defaultValue).then(res => {
      this.pipelineRuns = res.data
    })
    this.wsClient = NewClient()
    this.wsClient.onmessage = (e) => {
      if (e.data !== 'ping') {
        const result = JSON.parse(e.data)
        if (result.type === 'pipelineRun' && result.result.ns === this.defaultValue) {
          this.pipelineRuns = result.result.data
          this.$forceUpdate()
        }
      }
    }
